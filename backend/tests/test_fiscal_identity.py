...
def test_get_by_tax(db):
    ...
    assert fiscal_identity_crud.get_by_tax_id(db, "20182810674") == original_identity
    assert fiscal_identity_crud.get_by_tax_id(db, "11111111111") is None
...
def test_create_name(db):
    ...
    assert len(fiscal_identity_crud.get_all(db)) == 1
    ...
    with pytest.raises(DuplicateFiscalIdentityNameError):
        fiscal_identity_crud.create(db, fiscal)


def test_create_tax(db):
    fiscal1 = FiscalIdentityCreate(
        name="Test", condicion_iva=CondicionIva.INSCRIPTO, tax_id="11111111111"
    )
    fiscal_identity_crud.create(db, fiscal1)
    ...
    with pytest.raises(DuplicateFiscalIdentityTaxIdError):
        fiscal_identity_crud.create(db, fiscal2)
